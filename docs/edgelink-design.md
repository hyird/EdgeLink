...
└───────────────────────────────────┬─────────────────────────────────────────────┘
           ...
           │  Server      │  Interconnect │  Server      │ │ (via CDN)    │
           │ (Asia)       │               │ (US)         │ │ (EU)         │
           │              │               │              │ └──────────────┘
           │ Relay: WSS   │               │ Relay: WSS   │
           │ STUN: UDP    │               │ STUN: UDP    │
...
│                                                                                 │
│  关键实现:                                                                       │
│  1. Host Header: 设置为目标 Relay 的域名 (CDN 路由依据)                         │
│  2. SNI: SSL 握手时使用目标域名                                                 │
│  3. RTT 测量: 测量的是端到端实际延迟，包含 CDN 引入的延迟                       │
│                                                                                 │
│  RTT 测量改进:                                                                   │
│  - 使用 ping_id 追踪每个 PING 请求                                              │
│  - 使用 EMA (指数移动平均) 平滑延迟数据: avg = 0.3*cur + 0.7*old               │
│  - 记录 min/max/avg/current RTT                                                 │
│  - 超时清理: 30秒未响应的 PING 自动清理                                         │
│                                                                                 │
│  配置示例:                                                                       │
│  {                                                                              │
│    "mesh": {                                                                    │
│      "peers": [                                                                 │
│        "wss://relay-us.example.com/ws/mesh",     // 可能经过 CDN                │
...
│    │  session_key = HKDF(shared, "edgelink-session", A_id || B_id) │           │
│    │                               │                               │           │
...
```json
{
  ...
  "tls": {
    "cert": "/etc/edgelink/cert.pem",
    "key": "/etc/edgelink/key.pem"
  },
  "database": {
    "type": "sqlite",
    "path": "/var/lib/edgelink/mesh.db"
  },
  "jwt": {
