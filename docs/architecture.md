...
#### 3.4.13.2 ENDPOINT_UPDATE Payload (Type=0x46)

客户端上报自己的网络端点给 Controller，用于 P2P 打洞时提供端点信息。
...
| 字段    | 大小 | 说明                                           |
| ------- | ---- | ---------------------------------------------- |
| address | 4 B  | IPv4 地址 (网络字节序)                         |
| port    | 2 B  | 端口号                                         |
| type    | 1 B  | 端点类型: 0x01=LAN, 0x02=STUN, 0x03=UPNP, 0x04=RELAY |

**上报时机**：
- P2PManager 启动成功后立即上报
- 端点发生变化时重新上报（如 IP 变更、NAT 重映射）

**Controller 行为**：
- 收到后存储在内存中（按 node_id 索引）
- 会话断开时清除对应端点
- 处理 P2P_INIT 时返回存储的端点
...
Client 支持定时自动测量到所有在线对端节点的延迟 (RTT)，用于持续监控网络质量。
...
| 参数                    | 默认值 | 说明                              |
| ----------------------- | ------ | --------------------------------- |
| latency_measure_interval| 30s    | 延迟测量间隔 (0 = 禁用)           |

**实现细节**：
- 使用内部 Relay Ping 协议 (0xEE/0xEF) 测量 RTT
- 每个节点的 Ping 超时时间为 2 秒
- 节点之间间隔 100ms 发送 Ping，避免流量突发
- 测量结果自动更新到 `PeerManager` 中
- 跳过自身节点 (node_id == self)
- 定时器在 `stop()` 时取消
...
**设计原则**：
1. **Relay 立即可用**：连接 Controller 后立刻通过 Relay 通信，无需等待
2. **P2P 按需发起**：仅在首次向某 peer 发送数据时触发 P2P 连接
3. **无感知切换**：P2P 建立成功后，数据自动从 Relay 切换到 P2P
4. **故障自动降级**：P2P 失败自动回退到 Relay

**触发点**：

| 触发时机 | 触发条件 | 说明 |
| -------- | -------- | ---- |
| 首次发送数据 | P2P 未连接时调用 `send_to_peer()` | 触发 P2P 连接（异步，不阻塞发送）|
| peer 下线 | del_peer_ids | 断开对应的 P2P 连接 |

> **注意**：不再在收到 CONFIG 或 CONFIG_UPDATE 时自动对所有 peer 发起 P2P。
> 这避免了在 peer 较多时产生不必要的打洞流量，也避免了连接初期的竞态条件。

**双向打洞时序图** (参考 EasyTier 实现)：
...
| 步骤 | 描述 | 关键点 |
|------|------|--------|
| 1-2  | 两端上线并上报端点 | 只上报 STUN 发现的公网端点，过滤内网 IP |
| 3    | 首次发送数据触发 P2P | Relay 优先，P2P 异步 |
| 4    | A 向 Controller 发起 P2P_INIT | 请求 B 的端点信息 |
| 5    | **Controller 双向通知** | 同时发送两个 P2P_ENDPOINT |
| 6    | **双向同时打洞** | A→B 和 B→A 的 PING 同时发出 |
| 7    | 收到对方的 PING 后回复 PONG | 确认连接可用 |
| 8    | P2P 连接建立 | 后续数据直接 UDP 传输 |

> **为什么需要双向打洞？**
>
> 对于端口受限型 NAT (Port Restricted Cone NAT)，如果只有 A 向 B 发包，
...
send_to_peer(peer_id, data)
  ...
  [已连接]     [未连接]
      ...
      │             │
      ▼             ▼
 [P2P 发送]   [Relay 发送] + [异步打洞]
```

**幂等性保证**：

`connect_peer()` 是幂等操作，多次调用不会重复发起请求：
