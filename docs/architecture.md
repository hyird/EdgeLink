...
Client 采用 **Relay 优先、P2P 异步** 的连接策略，确保用户体验优先：

**设计原则**：
1. **Relay 立即可用**：连接 Controller 后立刻通过 Relay 通信，无需等待
2. **P2P 后台尝试**：异步发起 P2P 连接，成功后自动切换
...
| 触发时机 | 触发条件 | 说明 |
| -------- | -------- | ---- |
| 收到 CONFIG | peer.online && peer.node_id != self | 对所有在线 peer 发起 P2P |
| 收到 CONFIG_UPDATE | add_peers 中 online 的 peer | 新上线的 peer 自动发起 P2P |
| 首次发送数据 | P2P 未连接时 | 触发 P2P 连接（异步，不阻塞发送）|
...
on_config / on_config_update / send_to_peer
                    │
                    ▼
         ┌─────────────────────┐
         │ p2p_mgr_->connect_  │
