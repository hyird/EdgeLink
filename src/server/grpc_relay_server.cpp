...
GrpcRelayServer::GrpcRelayServer(const ServerConfig& config)
    : config_(config)
    , session_manager_(config.controller.token)  // Use server token for JWT validation
{}
...
void GrpcRelayServer::setup_ssl_credentials() {
    if (config_.relay.tls.enabled) {
        ...
        if (!cert_file.is_open()) {
            LOG_ERROR("GrpcRelayServer: Failed to open cert file: {}",
                      config_.relay.tls.cert_file);
        ...
        if (!key_file.is_open()) {
            LOG_ERROR("GrpcRelayServer: Failed to open key file: {}",
                      config_.relay.tls.key_file);
            credentials_ = grpc::InsecureServerCredentials();
            return;
        }
