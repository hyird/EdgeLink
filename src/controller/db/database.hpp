...
class Database {
public:
    // Row type for query results - maps column name to value
    ...
    template<typename T>
    std::vector<Row> query(const std::string& sql, T param) {
        return query_impl(sql, {to_value(param)});
    }

    // Execute a query with two parameters
    template<typename T1, typename T2>
    std::vector<Row> query(const std::string& sql, T1 p1, T2 p2) {
        return query_impl(sql, {to_value(p1), to_value(p2)});
    }

    // Execute a query with three parameters
    template<typename T1, typename T2, typename T3>
    std::vector<Row> query(const std::string& sql, T1 p1, T2 p2, T3 p3) {
        return query_impl(sql, {to_value(p1), to_value(p2), to_value(p3)});
    }

    // Generic execute with variadic parameters
    template<typename... Args>
    bool execute(const std::string& sql, Args... args) {
        return execute_impl(sql, {to_value(args)...});
    }
...
private:
    ...
    // Implementation for variadic execute/query
    bool execute_impl(const std::string& sql, const std::vector<Value>& params);
    std::vector<Row> query_impl(const std::string& sql, const std::vector<Value>& params);

    // Convert various types to Value
    static Value to_value(int v) { return static_cast<int64_t>(v); }
    static Value to_value(int64_t v) { return v; }
    static Value to_value(uint32_t v) { return static_cast<int64_t>(v); }
    static Value to_value(uint64_t v) { return static_cast<int64_t>(v); }
    static Value to_value(double v) { return v; }
    static Value to_value(const std::string& v) { return v; }
    static Value to_value(const char* v) { return std::string(v); }
