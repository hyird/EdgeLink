...
bool Client::init_relay_manager() {
    try {
        relay_manager_ = std::make_shared<RelayManager>(
            ioc_, ssl_ctx_, node_id_, relay_token_
        );
        
        RelayManagerCallbacks callbacks;
...
void Client::on_relay_state_changed(uint32_t relay_id, RelayConnection::State state) {
...
void Client::on_latency_report_timer() {
    ...
    if (relay_manager_) {
        ...
        for (uint32_t relay_id : connected_relays) {
            ...
            if (state == RelayConnection::State::CONNECTED) {
...
void Client::on_endpoints_changed(const std::vector<edgelink::Endpoint>& endpoints) {
    LOG_DEBUG("Endpoints changed, {} endpoints discovered", endpoints.size());
    
    // 转换为字符串格式上报
    std::vector<std::string> ep_strings;
    for (const auto& ep : endpoints) {
        ep_strings.push_back(ep.ip + ":" + std::to_string(ep.port));
    }
