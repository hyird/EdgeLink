...
public:
    ...
    std::shared_ptr<RelayManager> get_relay_manager() { return relay_manager_; }
...
private:
    ...
    void on_relay_state_changed(uint32_t relay_id, RelayConnection::State state);
    ...
    // EndpointManager回调
    void on_endpoints_changed(const std::vector<edgelink::Endpoint>& endpoints);
    ...
    std::shared_ptr<RelayManager> relay_manager_;
