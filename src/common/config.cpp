...
static void pretty_print_json(std::ostream& os, const boost::json::value& jv,
                               int indent = 0, int indent_size = 2) {
    std::string pad(indent, ' ');
    std::string pad_inner(indent + indent_size, ' ');
    ...
    switch (jv.kind()) {
        case boost::json::kind::object: {
            ...
            for (const auto& [key, val] : obj) {
                ...
                first = false;
                os << pad_inner << "\"" << key << "\": ";
                pretty_print_json(os, val, indent + indent_size, indent_size);
            }
            os << "\n" << pad << "}";
            break;
        }
...
bool ControllerConfig::save(const std::filesystem::path& path) const {
    ...
    pretty_print_json(file, boost::json::value(to_json()));
    file << "\n";
    return true;
}
...
std::optional<ClientConfig> ClientConfig::from_json(const boost::json::value& v) {
    try {
        ...
        if (obj.contains("auth_key")) cfg.auth_key = obj.at("auth_key").as_string().c_str();
...
bool KeyStorage::save(const std::filesystem::path& path) const {
    ...
    pretty_print_json(file, boost::json::value(obj));
    file << "\n";
    return true;
}
